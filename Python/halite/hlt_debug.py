...
class GameMap:
    ...
    def neighbors(self, square, n=1, include_self=False, star_shape=True):
        ...
        if n == 1:
            ...
            combos = ((dx, dy) for dy in range(-n, n+1) for dx in range(-n, n+1) if not star_shape or abs(dx) + abs(dy) <= n)
